...
export function DataMethodsProvider({ children }: PropsWithChildren) {
    ...
    const methods: DataMethods = useMemo(() => {
        ...
        const subscribeLatestArraysTyped = makeSubscriber(
            subscriptionsLatestArraysTyped.current,
        );
        ...
        console.warn("RECREATING METHODS!!!");
        return {
            subscribeReset,
            subscribeViewEdges,
...
export function useDataMethods() {
    ...
    if (!context) {
        throw new Error(
            "useDataMethods must be used within a DataSubscriptionProvider!",
        );
    }
    return context;
}
